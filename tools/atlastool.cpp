...
void GetLocales(const char *locales, std::vector<CharRange> &ranges)
{
	...
	for (size_t i = 0; i < strlen(locales); i++) {
		...
		case 'K':  // Korean (hangul)
			ranges.push_back(range(0x1100, 0x11FF, hangul1));
			ranges.push_back(range(0x3130, 0x318F, hangul2));
...
int main(int argc, char **argv) {
  ...
  fprintf(cpp_file, "// C++ generated by atlastool from %s (kaienfr@gmail.com)\n\n", argv[1]);
  ...
  fprintf(h_file, "// Header generated by atlastool from %s (kaienfr@gmail.com)\n\n", argv[1]);
